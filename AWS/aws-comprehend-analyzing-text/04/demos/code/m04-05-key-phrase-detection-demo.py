...
input_data_config = {
	...
	"InputFormat": "ONE_DOC_PER_LINE", # two possible options: ONE_DOC_PER_FILE | ONE_DOC_PER_LINE
}
...
response = api.start_key_phrases_detection_job(
	InputDataConfig=input_data_config,
	OutputDataConfig=output_data_config,
	DataAccessRoleArn=DATA_ACCESS_ARN,
	LanguageCode='en',
	JobName="M04-05 Key Phrase Detection Demo",
	)
...
polling = True
while polling:
	response = api.describe_key_phrases_detection_job(JobId=job_id)
	print("Current status of job: {}".format(response['KeyPhrasesDetectionJobProperties']['JobStatus']))
	if response['KeyPhrasesDetectionJobProperties']['JobStatus'] in [ 'SUBMITTED', 'IN_PROGRESS']:
		# Work in progress, wait and keep polling
		print("Job in progress, waiting 30 seconds...")
		time.sleep(30) # wait 30 seconds and try again
	else:
		polling = False
		if response['KeyPhrasesDetectionJobProperties']['JobStatus'] == 'FAILED':
			print("Job failed due to:\n\t{}".format(response['KeyPhrasesDetectionJobProperties']['Message']))
		elif response['KeyPhrasesDetectionJobProperties']['JobStatus'] in [ 'STOP_REQUESTED', 'STOPPED' ]:
			print("Job has been manually stopped")
		else:
			print("Job completed! Results are available at:\n{}\n".format(response['KeyPhrasesDetectionJobProperties']['OutputDataConfig']['S3Uri']))